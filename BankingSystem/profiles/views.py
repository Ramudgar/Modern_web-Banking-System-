...
def randomGen():
    ...
    return int(random.uniform(1000, 999999999))
...
def money_transfer(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            dest_user = randomGen()
            # models.Status.objects.get(account_number=dest_user_acc_num) # FIELD 1
...
def loan(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            purpose = models.Status.objects.get(user_name=request.user)  # FIELD
            loan_amount = curr_user.Amount  # FIELD 2


            # Now transfer the money!
            curr_user.balance = curr_user.balance + loan_amount

            # Save the changes before redirecting
            curr_user.balance.save()
            curr_user.save()
        ...
        loan = forms.Loan()
    return render(request, "profiles/loans.html", {"loan": loan})
